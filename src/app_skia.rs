//! The interface, ported from the gpui build in `app.rs`.
//!
//! Kept deliberately parallel to that file -- same order, same helpers, same
//! comments -- so the two can be diffed side by side while parity is signed off.
...
/// already hands back plain `futures::channel` receivers from threads it spawns
...
/// they came from.
#[derive(Clone)]
pub struct Updates(futures::channel::mpsc::UnboundedSender<Update>);

impl Updates {
    pub fn channel() -> (Updates, futures::channel::mpsc::UnboundedReceiver<Update>) {
...
impl RustyDlp {
    ...
    fn open_store(&mut self) {
        self.updates.spawn(|updates| {
            ...
            // turso's futures are runtime-agnostic, so a bare block_on drives
            // them with no executor involved.
    ...
    fn delete_job(&mut self, job_id: &str) {
        ...
        if let Some(store) = self.store.clone() {
            ...
            self.updates.spawn(move |_| {
                if let Err(e) = pollster::block_on(store.delete_job(&job_id)) {
    ...
    fn persist(&self, job: Job) {
        ...
        self.updates.spawn(move |_| {
            if let Err(e) = pollster::block_on(store.save_job(&job)) {
    ...
    fn save_editing(&mut self) {
        ...
        if let Some(store) = self.store.clone() {
            self.updates.spawn(move |_| {
                if renamed {
                    let _ = pollster::block_on(store.delete_preset(&previous_name));
                }
                if let Err(e) = pollster::block_on(store.save_preset(&preset)) {
    ...
    fn delete_editing(&mut self) {
        ...
        if let Some(store) = self.store.clone() {
            ...
            self.updates.spawn(move |_| {
                let _ = pollster::block_on(store.delete_preset(&name));
            });
        }
    }
