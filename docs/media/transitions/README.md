...
`status-chip.png` and `dialogs.png` are stills, not motion: the detail pane's
live status line in each phase a job passes through, and the two dialogs in the
states worth looking at. The rest are one sheet per animation, sampled six times
...
cargo test --lib -- --ignored dialog_states
```
